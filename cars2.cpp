...
int main () {
  ...
  *car = *GetCar(dataIn);
  while (dataIn) {
    car->price = car->price * 1.10;       //allows ptr to be used to point to address of C
    WriteCar(car);                            //allows ptr to be used to point to address of C
    carCount++;
    delete car;
    *car = *GetCar(dataIn);                  //allows ptr to be used to point to address of C
  }
